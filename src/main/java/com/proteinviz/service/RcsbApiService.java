 ...
 * APIs: Search, Entry metadata, Polymer entity, FASTA sequences, Citations, PDB file download.
 */
@Service
public class RcsbApiService {
    ...
    public String resolveQuery(String query) throws ProteinNotFoundException {
        if (query == null || query.isBlank()) throw new ProteinNotFoundException("Empty query");
        String t = query.trim().toUpperCase();
        if (t.matches("^[A-Z0-9]{4}$")) return t;
     ...
     *  3. Route conflict with metadata endpoint → caller must use /api/protein/pdb/{id}
     */
    public String fetchPdbFile(String pdbId) throws Exception {
        ...
        // Validate: real PDB files start with HEADER or contain ATOM records
        ...
        if (content.contains("<!DOCTYPE") || content.contains("<html") || content.contains("404")) {
            throw new ProteinNotFoundException("PDB structure not available for: " + id +
                    " (may not have a legacy PDB file — try a different ID)");
        }
    ...
    public List<SearchResult> searchProteins(String query, int max) throws IOException {
        ...
        try {
            ...
            if (rs.isArray()) for (JsonNode n : rs) {
                String id = n.path("identifier").asText().toUpperCase();
                if (!id.isBlank()) out.add(new SearchResult(id, n.path("score").asDouble()));
            }
        } catch (Exception e) { log.error("Search parse: {}", e.getMessage()); }
        return out;
    }
    ...
    private void fetchEntryMetadata(String id, ProteinStructure s) {
        try {
            ...
            if (!refine.isMissingNode()) {
                ...
                if (!res.isBlank()) s.setResolution(res + " Å");
            }
            if (s.getResolution() == null) s.setResolution("N/A");
            ...
            if (mw > 0) s.setMolecularWeight(String.format("%.1f kDa", mw / 1000.0));
            ...
            if (!kw.isBlank()) for (String k : kw.split(",")) kws.add(k.trim());
            s.setKeywords(kws);
            ...
            if (cites.isArray()) {
                ...
                for (JsonNode c : cites) {
                    ...
                    if (al.isArray()) for (JsonNode a : al) {
                        ...
                        if (!nm.isEmpty()) authors.add(nm);
                    }
                    p.setAuthors(authors);
                    if (p.getTitle() != null || p.getDoi() != null) pubs.add(p);
                }
                s.setPublications(pubs);
            }
        } catch (Exception e) {
            log.error("Entry metadata failed {}: {}", id, e.getMessage());
            if (s.getTitle() == null) s.setTitle("Structure " + id);
        }
    }
    ...
    private void fetchPolymerEntityData(String id, ProteinStructure s) {
        try {
            ...
            if (!srcOrg.isMissingNode()) {
                ...
                if (!sci.isBlank()) s.setScientificName(sci);
                int tax = srcOrg.path("ncbi_taxonomy_id").asInt(0);
                if (tax > 0) s.setTaxId(String.valueOf(tax));
            ...
            if (!desc.isBlank()) s.setFunctionDescription(desc);
    ...
    private void fetchSequences(String id, ProteinStructure s) {
        try {
            ...
            if (fasta == null || fasta.isBlank()) return;
            ...
            if (s.getChains() == null || s.getChains().isEmpty()) {
                ...
                for (SequenceChain sc : chains) if (sc.getChainId() != null) ids.add(sc.getChainId());
                s.setChains(ids);
            }
        } catch (Exception e) {
    ...
    private List<SequenceChain> parseFasta(String fasta) {
        ...
        for (String line : fasta.split("\n")) {
            line = line.trim();
            if (line.startsWith(">")) {
                if (cur != null) { cur.setSequence(seq.toString()); out.add(cur); }
                ...
                if (pipes.length >= 3) {
                    ...
                    if (!desc.isBlank()) cur.setDescription(desc);
                }
            } else if (cur != null && !line.isEmpty()) {
                seq.append(line.toUpperCase());
            }
        }
        if (cur != null && seq.length() > 0) { cur.setSequence(seq.toString()); out.add(cur); }
        return out;
    }
    ...
    private String searchByName(String query) throws ProteinNotFoundException {
        try {
            ...
            if (r.isEmpty()) throw new ProteinNotFoundException("No result for: " + query);
    ...
    private String httpGet(String url) throws IOException {
        try {
            ...
            if (resp.statusCode() == 404) throw new IOException("404: " + url);
    ...
    private String httpGetPdb(String url) throws IOException {
        try {
            HttpClient pdbClient = HttpClient.newBuilder()
            ...
            HttpRequest req = HttpRequest.newBuilder()
                    ...
                    .header("Accept-Encoding", "identity")   // disable gzip to get raw text
                    .header("User-Agent", "ProteinViz/1.0 (educational; contact info@proteinviz.dev)")
                    .GET().build();
            HttpResponse<String> resp = pdbClient.send(req, HttpResponse.BodyHandlers.ofString());
            if (resp.statusCode() == 200) return resp.body();
    ...
    public static class ProteinNotFoundException extends Exception {
        public ProteinNotFoundException(String msg) { super(msg); }
    }
}
